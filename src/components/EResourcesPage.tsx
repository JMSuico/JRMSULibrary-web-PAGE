...
const LinkTable: React.FC<LinkTableProps> = ({ title, links }) => (
  <div className="rounded-2xl p-6 shadow-lg border border-outline-variant" style={{ background: 'rgba(0,24,81,0.08)', backdropFilter: 'blur(4px)' }}>
    <h3 className="font-headline-md font-bold text-xl mb-4" style={{ color: '#001851', textShadow: '0 1px 4px rgba(255,255,255,0.5)' }}>{title}</h3>
    <div className="space-y-1">
      {links.map((link, idx) => (
        ...
        >
          ...
          <span className="text-sm group-hover:text-primary transition-colors flex-1" style={{ color: '#001851' }}>
            {link.name}
          </span>
