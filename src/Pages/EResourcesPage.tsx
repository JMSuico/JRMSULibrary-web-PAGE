...
const LinkTable: React.FC<LinkTableProps> = ({ title, links }) => (
  <div className="bg-primary/95 backdrop-blur-md rounded-2xl p-6 shadow-lg border border-gold-light/20">
    <h3 className="font-headline-md font-bold text-xl text-gold-light mb-4">{title}</h3>
    <div className="space-y-1">
      {links.map((link, idx) => (
        <a
          ...
          className="flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-gold-light/10 transition-colors group"
        >
          <span className="material-symbols-outlined text-gold-light text-lg">link</span>
          <span className="text-sm text-white/80 group-hover:text-gold-light transition-colors flex-1">
            {link.name}
          </span>
          <span className="material-symbols-outlined text-white/30 text-sm group-hover:translate-x-0.5 transition-transform">
            arrow_forward
          </span>
        </a>
      ))}
